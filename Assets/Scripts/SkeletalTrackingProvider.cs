        ...
        try
        {
            ...
            using (Device device = Device.Open(id))
            {
                device.StartCameras(new DeviceConfiguration()
                {
                    ...
                    DepthMode = DepthMode.NFOV_Unbinned, //可以改成廣角:DepthMode.WFOV_2x2Binned / DepthMode.WFOV_2x2Binned
                ...
                using (Tracker tracker = Tracker.Create(deviceCalibration, new TrackerConfiguration() { ProcessingMode = TrackerProcessingMode.Gpu, SensorOrientation = SensorOrientation.Default }))
                {
                    ...
                    while (!token.IsCancellationRequested)
                    {
                        ...
                        using (Frame frame = tracker.PopResult(TimeSpan.FromMilliseconds(100), throwOnTimeout: false))
                        {
                            if (frame == null)
                            {
